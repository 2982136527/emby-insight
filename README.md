...
### 6. 🎬 智能媒体刮削 (Smart Scraper)
- **精准识别**：基于文件名的智能解析，自动清理技术后缀（如 4K, HDR, Remux, 动态去除文件名中的文件大小信息），支持识别中文标题和年份。
- **智能提取**：针对电影和剧集优化了从文件夹结构提取标题的逻辑，自动处理合集（Collection）和嵌套文件夹，优先使用含年份的标准命名。
- **手动匹配**：提供强大的手动搜索弹窗，自动从文件路径提取最优关键词，支持与 TMDB 实时在线搜索和结果缓存。
- **刮削记录管理**：完整的刮削历史记录，支持按状态（已匹配/未匹配）和类型（电影/剧集）筛选，提供一键清空未匹配记录功能以支持重新刮削。
